...
+ (void)singleImagePicker:(UIViewController *)vc Crop:(BOOL)crop Circle:(BOOL)circle Radius:(CGFloat)radius Block:(WGImagePickerSingleBlock)block {
    ...
    if (selfVC.isTakeVideo && selfVC.isTakePhoto) {
        takePhotoTitle = @"相机";
    } else if (selfVC.isTakeVideo) {
        takePhotoTitle = @"拍摄";
    }
    ...
    UIAlertAction *imagePickerAction = [UIAlertAction actionWithTitle:@"去相册选择" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [selfVC pushTZImagePickerController];
    }];
...
- (instancetype)manyImagePicker:(NSInteger)count Block:(WGImagePickerManyBlock)block {
    [self initValue];
    self.maxCount = count;
    self.manyBlock = block;
    return self;
}
